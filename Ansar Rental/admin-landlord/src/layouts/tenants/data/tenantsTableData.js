...
export const TenantTable = (props) => {
    ...
    const unprocessedData = searchQuery.length !== 0 ? [...query.data.data].filter(d => d.username.toLowerCase().includes(searchQuery.toLowerCase())) : query.data.data

    const data = formatData(unprocessedData, deleteMutation, editMutation, openEditId, setOpenEditId, openDeleteId, setOpenDeleteId);

    return (<Stack gap={2}>
        <Stack direction='row' sx={{mt: 2, ml: 2}}>
            <TextField inputProps={{style: {fontSize: 16}}} InputLabelProps={{style: {fontSize: 16}}} variant='outlined' size='small' label='Search Tenants...'  onChange={(e) => {
                    ...
                    actions: (
                        <>
                            <SuiBox display="flex" flexDirection="row">
                                <SuiBox mx={4}>
                                    <SuiButton buttonColor="secondary" iconOnly={true}
                                               onClick={() => setOpenEditId(author.id)}>
                                        <Icon>edit</Icon>
                                    </SuiButton>
                                </SuiBox>
                                <SuiButton
